<template>
      ...
      <VCard class="auth-card pa-4 pt-5">
        <VCardItem class="align-left">
          ...
          <VCardTitle class="text-2xl font-weight-bold">
            Daftar File Berdasarkan Kategori : {{ categoryProfile.name ?? "-" }}
          </VCardTitle>
        </VCardItem>
        ...
        >
          ...
          <template #item-file.positions="item">
            <v-chip-group selected-class="text-primary" column>
              <div v-for="(x, index) in item.file.positions" :key="index">
  ...
  export default {
    ...
    data() {
      return {
        ...
        headers: [
          ...
          { text: "Kategori", value: "file.positions", sortable: true },
    ...
    methods: {
      ...
      async insertData() {
        try {
          ...
          if (response.status === 200) {
            this.closeModal();
      this.getAllFilePerCategory(this.categoryId);
      ...
      async getAllFilePerCategory(id: any) {
        try {
          const response = await mainURL.get(`/filepercategoryid/${id}`);
          if (response.status === 200) {
            this.items = response.data.data.file;
            this.categoryProfile = response.data.data.category;
          } else {
